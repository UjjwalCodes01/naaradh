...
export default async function LegalPage({ params }: { params: Params }) {
  ...
  return (
    <article className="prose-legal max-w-3xl">
      ...
      <p className="mt-2 text-sm text-slate-600">{page.summary}</p>
      <p className="mt-1 text-xs text-slate-500">Last updated {page.updated}</p>
      {page.draft ? (
        <p className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
          Draft — pending review by counsel. It describes how the product works today; binding terms
          follow legal review.
        </p>
      ) : null}
      {page.sections.map((s) => (
