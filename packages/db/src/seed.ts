 ...
 *
 * Every phone number here is from packages/shared/test/fake-phones.ts and passes through
...
/** Mirrors packages/pipeline hashPincode (the pipeline depends on this package, not the reverse). */
...
try {
  await db.transaction(async (tx) => {
    ...
    // Draft scripts: the disclosure validator (packages/scripts) is what approves them.
    await tx
      .insert(s.scripts)
      .values([
        {
    ...
    await tx
      ...
      .values({
        ...
        status: 'active',
        locale: 'hi-IN',
        // Same text as DEFAULT_INBOUND_GREETINGS['hi-IN'] in packages/scripts — disclosure first.
        greeting:
