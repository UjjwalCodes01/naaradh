...
describe('consent wording', () => {
  ...
  it('the checkout extension shows exactly the current wording and writes its version', () => {
    const ext = JSON.parse(
      read('apps/shopify/extensions/call-consent-checkout/consent-wording.json'),
    ) as {
      attribute: string;
      version: string;
    ...
    const source = read('apps/shopify/extensions/call-consent-checkout/src/Checkout.jsx');
  ...
  it('the cart block shows exactly the current wording and writes its version', () => {
    const liquid = read('apps/shopify/extensions/call-consent-cart/blocks/call-consent.liquid');
