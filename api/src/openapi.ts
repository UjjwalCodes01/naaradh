...
import { TOOL_NAMES, USE_CASES } from '@naaradh/call-scripts';
...
/** pipeline/src/intents.ts `SkipReason` (a type, so listed here). */
const SKIP_REASONS = [
  'use_case_disabled',
  'test_order',
  'staff_customer',
...
/** api/src/errors.ts STATUS_BY_CODE — the status each error code answers with. */
...
export function buildOpenApiDocument(): OpenAPIObject {
  ...
  return {
    ...
    servers: [
      ...
      { url: 'http://localhost:3001', description: 'Local development (api default PORT)' },
    ],
    tags: TAGS,
    security: [{ secretKey: [] }],
    paths: {
