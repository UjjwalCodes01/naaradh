...
function defaultPoolError(error: Error): void {
  // Structured for Cloud Logging when no logger was given (db has no pino dependency).
...
export function createDb(options: CreateDbOptions): {
  ...
  // db/test/int/rls.test.ts asserts the database setting.
  ...
  // and the process dies with a green health check (found by workers/test/int/chaos.test.ts).
