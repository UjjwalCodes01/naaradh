...
export interface AgentSpec {
  ...
  /** Rendered from an approved, immutable script version in call-scripts. */
  ...
  /**
   * The structured result the call must end with (call-scripts extraction schemas), as flat
...
export interface VoiceEngineAdapter {
  ...
  // --- Inbound + mid-call tools (ADR-0006). The adapter TRANSLATES; voice DECIDES. ---
...
export interface InboundAttachment {
  readonly e164: string;
  /** Our inbound-context URL for this number (voice builds and signs it). */
...
export interface ToolDefinition {
  ...
  /** JSON Schema for the arguments, generated from the Zod schema in call-scripts. */
