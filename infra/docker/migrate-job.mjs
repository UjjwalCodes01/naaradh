...
//
// Bundled by workers/Dockerfile into /app/dist/migrate.js with tsup (config next to this
...
//
// This file owns the "run it" decision. db/src/migrate.ts only exports runMigrations();
// the local CLI lives in migrate-cli.ts, so no bundle that imports it ever runs migrations.
import { runMigrations } from '../../db/src/migrate.ts';
